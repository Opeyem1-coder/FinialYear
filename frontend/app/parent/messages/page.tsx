...
import { Send, Search, PenSquare } from 'lucide-react'
...
interface User { _id: string; firstName: string; lastName: string; role: string }
...
export default function ParentMessagesPage() {
    ...
    const loadMessages = async () => {
        try {
            ...
            if (res.isMock || inMock) {
                setContacts([
                    { id: '1', name: 'Dr. John Smith', avatar: 'JS', relationship: 'Lecturer', lastMessage: 'Great progress on recent exams!', timestamp: new Date().toISOString(), unread: true },
                    { id: '2', name: 'Prof. Emily Davis', avatar: 'ED', relationship: 'Lecturer', lastMessage: 'Quarterly report is ready', timestamp: new Date(Date.now() - 3600000).toISOString(), unread: false },
                ])
                setAllMessages([
                    { _id: 'mock-m1', senderId: { _id: '1', firstName: 'John', lastName: 'Smith', role: 'teacher' }, receiverId: { _id: 'me' }, body: 'Great progress on recent exams!', createdAt: new Date().toISOString(), isRead: false },
                    { _id: 'mock-m2', senderId: { _id: '2', firstName: 'Emily', lastName: 'Davis', role: 'teacher' }, receiverId: { _id: 'me' }, body: 'Quarterly report is ready', createdAt: new Date(Date.now() - 3600000).toISOString(), isRead: true },
                ])
                return
            }
            ...
            msgs.forEach((m: any) => {
                ...
                const existing = contactsMap.get(contactUser._id)
                const isNewer = !existing || new Date(m.createdAt) > new Date(existing.timestamp)
                if (!existing || isNewer) {
                    contactsMap.set(contactUser._id, {
                        id: contactUser._id,
                        name: `${contactUser.firstName} ${contactUser.lastName}`,
                        avatar: `${contactUser.firstName?.[0] || ''}${contactUser.lastName?.[0] || ''}`,
                        relationship: contactUser.role === 'teacher' ? 'Lecturer' : 'User',
                        lastMessage: m.body, timestamp: m.createdAt,
                        unread: !iAmSender && !m.isRead
                    })
                }
            })
    ...
    const loadTeachers = async () => {
        try {
            ...
            if (!res.isMock) setTeachers(res.data.data.filter((u: any) => u.role === 'teacher'))
        } catch { }
    }
    ...
    const chatMessages: ChatMessage[] = allMessages
        ...
        .map(m => {
            ...
            return { id: m._id, sender: isMe ? 'You' : m.senderId?.firstName || 'User', content: m.body, timestamp: new Date(m.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), isMe, createdAt: m.createdAt }
        })
        .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())

    const handleSend = async () => {
        if (!messageText.trim() || !selectedContact) return
        if (inMock) { toast.warning('Switch to Live API to send messages.'); return }
        setIsSending(true)
        try {
            ...
            if (!res.isMock) {
                const uData = JSON.parse(sessionStorage.getItem('user') || '{}')
                setAllMessages(prev => [...prev, { ...res.data.data, senderId: { _id: getMyId(uData), firstName: 'You' }, receiverId: { _id: selectedContact } }])
    ...
    const handleCompose = async () => {
        ...
        try {
            ...
            toast.success('Message sent to lecturer!')
    ...
    useEffect(() => {
        ...
        allMessages.filter(m => m.senderId?._id === selectedContact && !m.isRead).forEach(m => {
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div><h1 className="text-4xl font-heading font-bold text-foreground mb-2">Messages</h1><p className="text-muted-foreground">Contact your child's lecturers directly</p></div>
                <PillButton onClick={() => setIsComposeOpen(true)}><PenSquare className="h-4 w-4 mr-2" />Message a Lecturer</PillButton>
            </div>

            <Dialog open={isComposeOpen} onOpenChange={setIsComposeOpen}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Message a Lecturer</DialogTitle><DialogDescription>Send a message directly to one of your child's lecturers</DialogDescription></DialogHeader>
                    <div className="space-y-4">
                        ...
                        <div><label className="text-sm font-medium">Subject</label><Input className="mt-1" placeholder="e.g. Regarding my child's attendance" value={composeSubject} onChange={e => setComposeSubject(e.target.value)} /></div>
                                    ...
                                    <div className="flex items-start gap-2">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between gap-1">
                                                ...
                                                {c.unread && <Badge variant="destructive" className="h-5 px-1 text-[10px]">New</Badge>}
                                            </div>
                    ...
                    <Card className="lg:col-span-2 overflow-hidden flex flex-col">
                        <CardHeader className="pb-3 border-b">
                            <div className="flex items-center gap-3">
                                <Avatar><AvatarFallback className="bg-primary/10 text-primary">{selectedContactData.avatar}</AvatarFallback></Avatar>
                                <div><CardTitle className="text-base">{selectedContactData.name}</CardTitle><CardDescription>{selectedContactData.relationship}</CardDescription></div>
                            </div>
                        </CardHeader>
                        ...
                        <div className="border-t p-4">
                            <div className="flex gap-2 items-end">
                                <Textarea placeholder="Type your message... (Enter to send)" value={messageText} onChange={e => setMessageText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend() } }} className="min-h-[40px] max-h-32" />
                                <PillButton onClick={handleSend} disabled={!messageText.trim() || isSending} size="icon"><Send className="h-5 w-5" /></PillButton>
                            </div>
                        </div>
                    </Card>
                ) : (
