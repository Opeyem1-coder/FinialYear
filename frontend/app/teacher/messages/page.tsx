...
export default function TeacherMessagesPage() {
    ...
    const loadUsers = async () => {
        try {
            ...
            if (!res.isMock) {
                setAllUsers(res.data.data.filter((u: any) => u.role === 'parent' || u.role === 'student'))
            }
        } catch { }
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div><h1 className="text-4xl font-heading font-bold text-foreground mb-2">Messages</h1><p className="text-muted-foreground">Communicate with parents and students</p></div>
                <PillButton onClick={() => setIsComposeOpen(true)}><PenSquare className="h-4 w-4 mr-2" />New Message</PillButton>
            </div>

            {/* Compose Dialog */}
            <Dialog open={isComposeOpen} onOpenChange={setIsComposeOpen}>
                <DialogContent>
                    <DialogHeader><DialogTitle>New Message</DialogTitle><DialogDescription>Send a message to a parent or student</DialogDescription></DialogHeader>
                    <div className="space-y-4">
                        ...
                        <div><label className="text-sm font-medium">Subject</label><Input className="mt-1" placeholder="e.g. Regarding attendance" value={composeSubject} onChange={e => setComposeSubject(e.target.value)} /></div>
                    ...
                    <DialogFooter>
                        ...
                        <PillButton onClick={handleCompose} disabled={isSending}><Send className="h-4 w-4 mr-2" />{isSending ? 'Sending...' : 'Send'}</PillButton>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
                ...
                <Card className="lg:col-span-1 overflow-hidden flex flex-col">
                    ...
                    <CardContent className="flex-1 overflow-hidden p-4 flex flex-col gap-3">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-2">
                            ...
                            {filteredContacts.length === 0 && <p className="text-center text-xs text-muted-foreground pt-4">No conversations yet. Click "New Message" to start one.</p>}
                        </div>
                    </CardContent>
                </Card>
                ...
                {selectedContactData ? (
                    <Card className="lg:col-span-2 overflow-hidden flex flex-col">
                        ...
                        <div className="border-t p-4">
                            <div className="flex gap-2 items-end">
                                <Textarea placeholder="Type your message... (Enter to send, Shift+Enter for new line)" value={messageText} onChange={e => setMessageText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend() } }} className="min-h-[40px] max-h-32" />
                    ...
                    <Card className="lg:col-span-2 flex items-center justify-center">
                        <div className="text-center space-y-2">
                            ...
                            <PillButton onClick={() => setIsComposeOpen(true)} className="mt-2"><PenSquare className="h-4 w-4 mr-2" />New Message</PillButton>
                        </div>
                    </Card>
                )}
            </div>
