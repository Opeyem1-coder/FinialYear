            ...
            try {
                ...
                if (isMock) {
                    if (mounted) {
                        ...
                        setCourses([
                            { courseName: 'CSC 301 — Data Structures', subject: '3 Units' },
                            { courseName: 'CSC 305 — Operating Systems', subject: '3 Units' },
                            { courseName: 'MTH 201 — Linear Algebra', subject: '4 Units' }
                        ])
                        setGrades([
                            { assignmentName: 'Mid-Semester Exam', courseId: { courseName: 'CSC 301' }, score: 85 },
                            { assignmentName: 'Quiz 2', courseId: { courseName: 'CSC 305' }, score: 78 }
                        ])
                ...
                // 1. Fetch student profile via userId
                ...
                if (!studentRes.isMock && studentRes.data?.data?.length > 0) {
                    ...
                    if (profile.courseIds) {
                        setCourses(profile.courseIds)
                    }
                } else if (!studentRes.isMock && studentRes.data?.data?.length === 0) {
                    // Fallback for registry created students mapping by email maybe? Or just generic fallback
                ...
                if (!gradesRes.isMock) {
                    // Mongoose populate usually doesn't populate courseId on grades if not explicitly defined, but we'll try to map it
                ...
                if (!msgRes.isMock) {
                    ...
                    // Count unread where receiver is me
